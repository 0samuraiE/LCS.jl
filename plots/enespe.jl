...
lηs = [0.0330, 0.0159, 0.00784, 0.00414, 0.00211]
...
for fontsize in (16, 18, 20, 24, 32)
    ...
    for (N, ν, Reλ, lη, ϵ, color) in zip(Ns, νs, Reλs, lηs, ϵs, KOLMOGOROV_COLORS)
        x = (1:(div(N, 2) + 1)) .* lη
        y = jldopen("data/enespe$N.h5", "r")["data"]
        y = y / (ϵ * ν^5)^(1 / 4)
    ...
    mkpath("figs")
    save("figs/kolmogorov-scaling-$fontsize.pdf", f)
    fontsize == 24 && save("figs/kolmogorov-scaling.pdf", f)
end
